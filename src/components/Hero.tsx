...
export const Hero = ({ children, ...props }: Props) => {
  let height: string = props.variant === "deep" ? "h-60 sm:h-96 md:h-100" : "h-32 sm:h-48 md:h-72";
  let heroClass: string = "w-full object-cover";
  return (
    <div className="relative flex items-center justify-center" {...props}>
      {children ? <h1 className="absolute text-7xl font-bold text-white">{children}</h1> : null}
      <img
